import { productRow } from "../Data Components/sellerProductRow.js";
...
{
  const baseUrl = 'https://69b10cdeadac80b427c3d349.mockapi.io';
  ...
  const targetSellerId = sellerIdParam || (loggedInUser && loggedInUser.role === 'seller' && loggedInUser.id ? String(loggedInUser.id) : null);

  let productsAPI = [];
  try {
      const res = await fetch(`${baseUrl}/products`, { method: 'GET', headers: { 'content-type': 'application/json' } });
      if (res.ok) {
          const all = await res.json();
          productsAPI = Array.isArray(all) ? all : [];
          if (targetSellerId) {
